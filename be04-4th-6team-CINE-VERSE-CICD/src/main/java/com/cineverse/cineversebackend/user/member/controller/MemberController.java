...
public class MemberController {
    ...
    /* 설명. 회원 생성 */
    @PostMapping("/regist")
    @Operation(summary = "회원 등록", description = "회원 정보를 등록 합니다.")
    @ApiResponse(responseCode = "200", description = "성공")
    @ApiResponse(responseCode = "400", description = "파라미터 오류")
    private ResponseEntity<Member> registMember(
            @Parameter(required = true, description = "회원 정보 등록요청")
    ...
    public ResponseEntity<String> login(
            @Parameter(required = true, description = "회원 로그인 요청")
    ...
    @PostMapping("/logout")
    @Operation(summary = "로그아웃", description = "회원 로그아웃을 요청합니다..")
    @ApiResponse(responseCode = "200", description = "성공")
    ...
    @Operation(summary = "회원 수정", description = "회원 정보를 수정 합니다.")
    @ApiResponse(responseCode = "200", description = "성공")
    @ApiResponse(responseCode = "400", description = "파라미터 오류")
    public ResponseEntity<Member> modifyMember(
            @Parameter(required = true, description = "회원 정보 수정요청")
            @RequestBody MemberDTO memberDTO ,
            @Parameter(required = true, description = "회원 고유번호")
            @PathVariable int memberId) {
    ...
    /* 설명. 회원 단일 조회 */
    @GetMapping("/{memberId}")
    @Operation(summary = "단일 회원 조회", description = "단일 회원 정보를 조회합니다.")
    @ApiResponse(responseCode = "200", description = "성공")
    @ApiResponse(responseCode = "400", description = "파라미터 오류")
    ...
    /* 설명. 회원 아이디 찾기 */
    @PostMapping("/find_id")
    @Operation(summary = "회원 아이디 찾기", description = "회원 아이디를 조회합니다.")
    @ApiResponse(responseCode = "200", description = "성공")
    @ApiResponse(responseCode = "400", description = "파라미터 오류")
    public ResponseEntity<String> findUserId(
            @Parameter(required = true, description = "회원 아이디 요청")
            @RequestBody MemberDTO memberDTO) {
